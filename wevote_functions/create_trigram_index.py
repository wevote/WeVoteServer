            ...
            with connection.cursor() as cursor:
                ...
                for field_name in fields:
                    ...
                    if _index_exists(cursor, index_name):
                        logger_func(f"Index already exists, skipping | index={index_name}")
                        continue
                    
                    create_index_sql = f"""CREATE INDEX {index_name} ON {table_name} USING gist ({field.column} gist_trgm_ops);"""
                    logger_func(f"Creating trigram index | index={index_name} | table={table_name} | column={field.column}")
                    
                    cursor.execute(create_index_sql)

        logger_func("Trigram index creation completed successfully.")
...
def _generate_safe_index_name(table_name, column_name):
    base_name = f"{column_name}_trgm_idx"
