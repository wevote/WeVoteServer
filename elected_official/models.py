...
class ElectedOfficialListManager(models.Model):
    ...
    def retrieve_all_candidates_for_office(self, office_id, office_we_vote_id):
        ...
        try:
            candidate_queryset = ElectedOfficial.objects.all()
            if positive_value_exists(office_id):
                candidate_queryset = candidate_queryset.filter(elected_office_id=office_id)
            elif positive_value_exists(office_we_vote_id):
                candidate_queryset = candidate_queryset.filter(elected_office_we_vote_id=office_we_vote_id)
            candidate_queryset = candidate_queryset.order_by('-twitter_followers_count')
            elected_official_list = candidate_queryset
    ...
    def retrieve_all_candidates_for_upcoming_election(self, google_civic_election_id=0, state_code='',
        ...
        try:
            ...
            candidate_queryset = candidate_queryset.order_by("elected_official_name")
            if positive_value_exists(google_civic_election_id):
                elected_official_list_objects = candidate_queryset
            else:
                elected_official_list_objects = candidate_queryset[:300]
    ...
    def retrieve_candidate_count_for_office(self, office_id, office_we_vote_id):
        ...
        try:
            candidate_queryset = ElectedOfficial.objects.using('readonly').all()
            if positive_value_exists(office_id):
                candidate_queryset = candidate_queryset.filter(elected_office_id=office_id)
            elif positive_value_exists(office_we_vote_id):
                candidate_queryset = candidate_queryset.filter(elected_office_we_vote_id=office_we_vote_id)
            ...
            candidate_count = elected_official_list.count()
            success = True
    ...
    def is_automatic_merge_ok(self, candidate_option1, candidate_option2):
        ...
        status = ""
        if candidate_option1.elected_official_name != candidate_option2.elected_official_name:
            automatic_merge_ok = False
            status += " elected_official_name:"
        candidate1_twitter_handle = str(candidate_option1.elected_official_twitter_handle)
        candidate2_twitter_handle = str(candidate_option2.elected_official_twitter_handle)
        if candidate1_twitter_handle.lower() != candidate2_twitter_handle.lower():
            automatic_merge_ok = False
            status += " elected_official_twitter_handle:"
        if candidate_option1.elected_official_url != candidate_option2.elected_official_url:
            automatic_merge_ok = False
    ...
    def retrieve_elected_officials_from_all_elections_list(self):
        ...
        """
        candidates_list_temp = ElectedOfficial.objects.all()
        ...
        candidates_list_temp = candidates_list_temp.order_by('elected_official_name')[:300]
        return candidates_list_temp

    def remove_duplicate_candidate(self, elected_official_id, google_civic_election_id):
    ...
    def retrieve_possible_duplicate_candidates(self, elected_official_name, google_civic_elected_official_name,
        ...
        try:
            ...
            # candidate_queryset = candidate_queryset.filter(elected_office_we_vote_id__iexact=office_we_vote_id)
    ...
    def retrieve_candidates_from_non_unique_identifiers(self, google_civic_election_id, state_code,
        ...
        if keep_looking_for_duplicates and positive_value_exists(elected_official_twitter_handle):
            try:
                candidate_query = ElectedOfficial.objects.all()
                candidate_query = candidate_query.filter(elected_official_twitter_handle__iexact=elected_official_twitter_handle,
                                                         google_civic_election_id=google_civic_election_id)
                if positive_value_exists(state_code):
                ...
                if positive_value_exists(ignore_elected_official_id_list):
                    candidate_query = candidate_query.exclude(we_vote_id__in=ignore_elected_official_id_list)

                elected_official_list = list(candidate_query)
                if len(elected_official_list):
        ...
        if keep_looking_for_duplicates and positive_value_exists(elected_official_name):
            ...
            try:
                candidate_query = ElectedOfficial.objects.all()
                candidate_query = candidate_query.filter(elected_official_name__iexact=elected_official_name,
                                                         google_civic_election_id=google_civic_election_id)
                if positive_value_exists(state_code):
                    candidate_query = candidate_query.filter(state_code__iexact=state_code)
        ...
        if keep_looking_for_duplicates and positive_value_exists(elected_official_name):
            ...
            try:
                ...
                candidate_query = candidate_query.filter(elected_official_name__icontains=first_name)
                last_name = extract_last_name_from_full_name(elected_official_name)
                candidate_query = candidate_query.filter(elected_official_name__icontains=last_name)
...
class ElectedOfficial(models.Model):
    ...
    # The internal We Vote id for the ElectedOffice that this elected official is competing for. During setup we need to allow
    # this to be null.
    ...
    elected_office_we_vote_id = models.CharField(
        verbose_name="we vote permanent id for the office this elected official is running for", max_length=255, default=None,
    ...
    elected_official_name = models.CharField(verbose_name="elected official name", max_length=255, null=False, blank=False)
    # The candidate's name as passed over by Google Civic. We save this so we can match to this elected official even
    ...
    # The order the elected official appears on the ballot relative to other candidates for this contest.
    ...
    # State code
    state_code = models.CharField(verbose_name="state this elected official serves", max_length=2, null=True, blank=True)
    ...
    elected_official_email = models.CharField(verbose_name="elected official email", max_length=255, null=True, blank=True)
    # The voice phone number for the elected officials office.
    elected_official_phone = models.CharField(verbose_name="elected official phone", max_length=255, null=True, blank=True)
    ...
    ballotpedia_elected_official_name = models.CharField(verbose_name="elected official name exactly as received from ballotpedia",
                                                  max_length=255, null=True, blank=True)
    ballotpedia_elected_official_url = models.URLField(verbose_name='url of elected official on ballotpedia', blank=True, null=True)
    ...
    def office(self):
        try:
            office = ElectedOffice.objects.get(id=self.elected_office_id)
...
def fetch_candidate_count_for_office(office_id=0, office_we_vote_id=''):
    ...
    results = elected_official_list.retrieve_candidate_count_for_office(office_id, office_we_vote_id)
    return results['candidate_count']


# See also 'convert_to_political_party_constant' in we_vote_functions/functions.py
def candidate_party_display(raw_political_party):
...
class ElectedOfficialManager(models.Model):
    ...
    def retrieve_elected_official_from_maplight_id(self, candidate_maplight_id):
        elected_official_id = 0
        we_vote_id = ''
        elected_official_manager = ElectedOfficialManager()
        return elected_official_manager.retrieve_elected_official(
            elected_official_id, we_vote_id, candidate_maplight_id)

    def retrieve_elected_official_from_vote_smart_id(self, candidate_vote_smart_id):
        elected_official_id = 0
        we_vote_id = ''
        ...
        elected_official_manager = ElectedOfficialManager()
        return elected_official_manager.retrieve_elected_official(
            elected_official_id, we_vote_id, candidate_maplight_id, elected_official_name, candidate_vote_smart_id)
    ...
    def retrieve_elected_official_from_elected_official_name(self, elected_official_name):
        ...
        results = elected_official_manager.retrieve_elected_official(
            elected_official_id, we_vote_id, candidate_maplight_id, elected_official_name)
        if results['success']:
            return results
        ...
        results = elected_official_manager.retrieve_elected_official(
            elected_official_id, we_vote_id, candidate_maplight_id, elected_official_name_try2)
        if results['success']:
            return results
        ...
        if elected_official_name_try3 != elected_official_name:
            results = elected_official_manager.retrieve_elected_official(
                elected_official_id, we_vote_id, candidate_maplight_id, elected_official_name_try3)
            if results['success']:
                return results
    ...
    def retrieve_elected_official(
            self, elected_official_id, elected_official_we_vote_id=None, candidate_maplight_id=None,
            elected_official_name=None, candidate_vote_smart_id=None, ballotpedia_elected_official_id=None):
        error_result = False
        ...
        try:
            if positive_value_exists(elected_official_id):
                ...
                elected_official_on_stage = ElectedOfficial.objects.get(maplight_id=candidate_maplight_id)
                elected_official_id = elected_official_on_stage.id
                elected_official_we_vote_id = elected_official_on_stage.we_vote_id
                elected_official_found = True
                status = "RETRIEVE_CANDIDATE_FOUND_BY_MAPLIGHT_ID"
            elif positive_value_exists(candidate_vote_smart_id):
                elected_official_on_stage = ElectedOfficial.objects.get(vote_smart_id=candidate_vote_smart_id)
                elected_official_id = elected_official_on_stage.id
                elected_official_we_vote_id = elected_official_on_stage.we_vote_id
                elected_official_found = True
    ...
    def retrieve_candidates_are_not_duplicates(self, elected_official1_we_vote_id, elected_official2_we_vote_id, for_editing=False):
        candidates_are_not_duplicates = ElectedOfficialsAreNotDuplicates()
        # Note that the direction of the friendship does not matter
        try:
            if positive_value_exists(for_editing):
                candidates_are_not_duplicates = ElectedOfficialsAreNotDuplicates.objects.get(
                ...
                candidates_are_not_duplicates = ElectedOfficialsAreNotDuplicates.objects.using('readonly').get(
                    elected_official1_we_vote_id__iexact=elected_official1_we_vote_id,
                    elected_official2_we_vote_id__iexact=elected_official2_we_vote_id,
                )
    ...
    def retrieve_candidates_are_not_duplicates_list(self, elected_official_we_vote_id, for_editing=False):
        """
        ...
        try:
            if positive_value_exists(for_editing):
                candidates_are_not_duplicates_list_query = ElectedOfficialsAreNotDuplicates.objects.filter(
                    elected_official1_we_vote_id__iexact=elected_official_we_vote_id,
                )
            else:
                candidates_are_not_duplicates_list_query = ElectedOfficialsAreNotDuplicates.objects.using('readonly').filter(
                    elected_official1_we_vote_id__iexact=elected_official_we_vote_id,
                )
        ...
        for one_entry in candidates_are_not_duplicates_list:
            if one_entry.elected_official1_we_vote_id != elected_official_we_vote_id:
                candidates_are_not_duplicates_list_we_vote_ids.append(one_entry.elected_official1_we_vote_id)
            elif one_entry.elected_official2_we_vote_id != elected_official_we_vote_id:
                candidates_are_not_duplicates_list_we_vote_ids.append(one_entry.elected_official2_we_vote_id)
        results = {
            'success':                                  success,
            'status':                                   status,
    ...
    def fetch_candidates_are_not_duplicates_list_we_vote_ids(self, elected_official_we_vote_id):
        results = self.retrieve_candidates_are_not_duplicates_list(elected_official_we_vote_id)
    ...
    def update_or_create_elected_official(self, elected_official_we_vote_id, google_civic_election_id, ocd_division_id,
        ...
        # DALE 2016-02-20 We are not requiring elected_office_id or elected_office_we_vote_id to match a candidate
        ...
        #     status = 'MISSING_CONTEST_OFFICE_ID'
            ...
            try:
                ...
                elected_official_on_stage, new_candidate_created = \
    ...
    def update_or_create_candidates_are_not_duplicates(self, elected_official1_we_vote_id, elected_official2_we_vote_id):
        """
        Either update or create a elected_official entry.
        """
        exception_multiple_object_returned = False
    ...
    def update_candidate_social_media(self, elected_official, elected_official_twitter_handle=False,
                                      candidate_facebook=False):
        """
        ...
        elected_official_twitter_handle = elected_official_twitter_handle.strip() if elected_official_twitter_handle else False
        ...
        if elected_official:
            ...
            if candidate_facebook:
                if candidate_facebook != elected_official.facebook_url:
                    elected_official.facebook_url = candidate_facebook
                    values_changed = True
    ...
    def update_candidate_twitter_details(self, elected_official, twitter_json, cached_twitter_profile_image_url_https,
        ...
        if elected_official:
            ...
            if positive_value_exists(cached_twitter_profile_background_image_url_https):
                elected_official.twitter_profile_background_image_url_https = cached_twitter_profile_background_image_url_https
                values_changed = True
    ...
    def reset_candidate_image_details(self, elected_official, twitter_profile_image_url_https,
                                      twitter_profile_background_image_url_https,
                                      twitter_profile_banner_url_https):
        """
    ...
    def clear_candidate_twitter_details(self, elected_official):
        """
    ...
    def refresh_cached_candidate_office_info(self, candidate_object):
        ...
        results = {}
        if positive_value_exists(candidate_object.elected_office_id):
            results = elected_office_manager.retrieve_elected_office_from_id(candidate_object.elected_office_id)
            office_found = results['elected_office_found']
        elif positive_value_exists(candidate_object.elected_office_we_vote_id):
            results = elected_office_manager.retrieve_elected_office_from_we_vote_id(
                candidate_object.elected_office_we_vote_id)
            office_found = results['elected_office_found']

        if office_found:
            office_object = results['elected_office']
            candidate_object.elected_office_id = office_object.id
            candidate_object.elected_office_we_vote_id = office_object.we_vote_id
            candidate_object.elected_office_name = office_object.office_name
            values_changed = True
    ...
    def create_candidate_row_entry(self, update_values):
        ...
        candidate_party_name = update_values['political_party'] if 'political_party' in update_values else ''
        ...
        if new_candidate_created:
            try:
                ...
                new_elected_official.political_party = candidate_party_name
                new_elected_official.ctcl_uuid = ctcl_uuid
                new_elected_official.candidate_is_incumbent = candidate_is_incumbent
                new_elected_official.candidate_is_top_ticket = candidate_is_top_ticket
                ...
                if new_elected_official.photo_url:
                    candidate_results = \
                        self.modify_candidate_with_organization_endorsements_image(new_elected_official,
                                                                                   photo_url, True)
                    if candidate_results['success']:
                        elected_official = candidate_results['candidate']
    ...
    def update_candidate_row_entry(self, elected_official_we_vote_id, update_values):
        """
        ...
        try:
            existing_candidate_entry = ElectedOfficial.objects.get(we_vote_id__iexact=elected_official_we_vote_id)
            values_changed = False

            if existing_candidate_entry:
                # found the existing entry, update the values
                if 'ballotpedia_elected_official_id' in update_values:
                    existing_candidate_entry.ballotpedia_elected_official_id = \
                        convert_to_int(update_values['ballotpedia_elected_official_id'])
                    values_changed = True
                if 'ballotpedia_elected_official_name' in update_values:
                    existing_candidate_entry.ballotpedia_elected_official_name = update_values['ballotpedia_elected_official_name']
                    values_changed = True
                if 'ballotpedia_elected_official_url' in update_values:
                    existing_candidate_entry.ballotpedia_elected_official_url = update_values['ballotpedia_elected_official_url']
                    values_changed = True
                ...
                if 'elected_official_name' in update_values:
                    existing_candidate_entry.elected_official_name = update_values['elected_official_name']
                    values_changed = True
                ...
                if 'elected_official_twitter_handle' in update_values:
                    existing_candidate_entry.elected_official_twitter_handle = update_values['elected_official_twitter_handle']
                    values_changed = True
                if 'elected_official_url' in update_values:
                    existing_candidate_entry.elected_official_url = update_values['elected_official_url']
                    values_changed = True
                if 'elected_office_we_vote_id' in update_values:
                    existing_candidate_entry.elected_office_we_vote_id = update_values['elected_office_we_vote_id']
                    values_changed = True
                if 'elected_office_id' in update_values:
                    existing_candidate_entry.elected_office_id = update_values['elected_office_id']
                    values_changed = True
                if 'elected_office_name' in update_values:
                    existing_candidate_entry.elected_office_name = update_values['elected_office_name']
                    values_changed = True
                ...
                if 'political_party' in update_values:
                    existing_candidate_entry.political_party = update_values['political_party']
                    values_changed = True
    ...
    def modify_candidate_with_organization_endorsements_image(self, elected_official, elected_official_photo_url,
    ...
    def count_candidates_for_election(self, google_civic_election_id):
        ...
        if positive_value_exists(google_civic_election_id):
            try:
                candidate_item_queryset = ElectedOfficial.objects.all()
