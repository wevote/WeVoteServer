...
1) Install Python 3.9, MacOS warns on 3.6 with a system level popup (in a MacOS "Terminal app window, ie. no virtual environments)
...
1) In the PyCharm IDE UI
    ...
    1) Open a **new** termanal window in the IDE, and run 'python --version' to double check that it is using Python 3.9
    1) Close the older terminal windows, that will have confused paths to the older python versions.
1) Get the new requirements.txt from git.
1) Get the latest setuptools
(PycharmEnvironments) stevepodell@Steves-MacBook-Pro-32GB-Oct-2109 WeVoteServer % pip3 install --upgrade setuptools   
1) Try requirements.txt in the Pycharm terminal window
...
1) This install requirements.txt worked
(PycharmEnvironments) stevepodell@Steves-MacBook-Pro-32GB-Oct-2109 WeVoteServer % pip3 install -r requirements.txt
